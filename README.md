...
You can think of a method as a **_subprogram—a _**small program inside of a program. 
...
The part of the program that creates a method is called a **_method definition. _**When a method is subsequently used in a program, we say that the definition is _called _or **_invoked. _**A single method definition may be called at many different points of a program.
...
* program limited to Fred (not Aaliyah or Ashanti)
...
<table>
  <tr>
   ...
   <td>related to the visibility to other programs, for our purposes we will set it to be <code>private</code> unless otherwise specified.  Other options are <code>public</code> or not specifying a modifier at all (<code>default</code>).
   </td>
  </tr>
  <tr>
   <td>return type
   </td>
   <td>indicates that data type of the value that the method will return.  A method that returns nothing has a return type of <code>void</code>  
   </td>
  </tr>
  <tr>
   <td>method name
   </td>
   <td>same naming conventions as variables.
   </td>
  </tr>
</table>
...
When the program runs, it executes the run() method.  The body of the run() method contains calls to the happy() method.  When this happens, the happy() body code is executed and then then control goes back to the run() method.
